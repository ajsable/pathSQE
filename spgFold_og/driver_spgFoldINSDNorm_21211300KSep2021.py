...
IPTS=21211
Instrument = 'ARCS'
OPdir='/SNS/{0}/IPTS-{1}/shared/Bander/10K/'.format(Instrument,IPTS)
Opdir='/SNS/{0}/IPTS-{1}/shared/Bander/10K/0.2CFold/'.format(Instrument,IPTS)
slicedir = Opdir+'ConstantESlices/'
OutputFilename='FeSi_10K_full.nxs'
processed_vanadium='/SNS/{0}/IPTS-{1}/shared/Dipanshu/van108467_new.nxs'.format(Instrument,IPTS)
van=Load(processed_vanadium)

########Let Things Load#####################################
merged3=LoadMD(OPdir+OutputFilename,LoadHistory=False) # Uncomment as necessary
M = mtd['merged3']
